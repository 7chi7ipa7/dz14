...
class SwaggerAPITest(unittest.TestCase):
    ...
    def test_get_userinfo01(self):
        "Получение информации о юзере, позитивный"
        self.assertEqual(status(swagger.user(1)), 200)
    def test_get_userinfo02(self):
        "Получение информации о юзере, негативный - не найден"
        self.assertEqual(status(swagger.pet(600)), 404)
    def test_get_pet_updfoto(self):
        "Обновление информации фото не потдерживает носитель"
        self.assertEqual(status(swagger.pet_upd2(1,"dog","asd")), 415) 
